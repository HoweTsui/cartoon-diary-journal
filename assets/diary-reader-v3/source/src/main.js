...
import { contain } from "./model.js";
import { onSpread, requestedSpread } from "./reader.js";
import {
  chooseDragDirection,
  dragFraction,
...
function resizeTextureImage(texture) {
  ...
  context.fillStyle = "#ffffff"; context.fillRect(0, 0, 900, 1200);
  const fit = contain(width, height, 900, 1200);
...
function pageMaterial(bookId, source) {
  ...
  const assetUrl = source.startsWith("data:") ? source : `${import.meta.env.BASE_URL}${source.replace(/^\/+/, "")}`;
  ...
  }).catch((error) => { console.error("Poster texture failed", source, error); document.querySelector("#render-note").textContent = "部分书页加载失败，请使用放大原图阅读。"; return blankMaterial; });
...
async function selectBook(book, immediate = false) {
  ...
  setLoading(true, `正在翻开《${book.title}》`);
  resize();
  ...
  flipBook.progress = requestedSpread();
...
function updateStatus() {
  if (loading || pointerStart || edgePreview.baseSheet !== null) return;
  ...
  onSpread(Math.round(shown / 2));
  if (shown === 0) pageState.textContent = "封面";
  else if (shown >= total) pageState.textContent = "封底";
  else pageState.textContent = `${Math.ceil(shown / 2)} / ${Math.ceil(total / 2) - 1}`;
...
window.addEventListener("keydown", (event) => {
  if (event.target.closest("input, select, textarea, button, a") || document.querySelector("dialog[open]")) return;
...
function animate(timestamp) {
  ...
  canvas.dataset.settled = String(Math.abs(flipBook.progress - flipBook.currentPage / 2) < 0.0001);
...
window.addEventListener("reader:goto", () => { canvas.dataset.settled = "false"; clearEdgePreview(true); flipBook.currentPage = requestedSpread()*2; requestRender(); });
new ResizeObserver(resize).observe(canvas.parentElement);
window.addEventListener("reader:cover", async () => {
  disposeBookMaterials(selectedBook.id);
  lastStatusKey = "";
  await selectBook(books[0], true);
});
