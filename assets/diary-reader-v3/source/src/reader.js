...
export function onSpread(value){
 ...
 $("#entry-title").textContent=e?.title||data.book.title;
 $("#period-label").textContent=e?e.date+" / "+(p?.title||""):value===0?data.entries.length+" 篇日记 · "+data.periods.length+" 个时期":"这一册，已读完";
 ...
 $("#enlarge").disabled=value>data.entries.length;
}
...
function renderResults(){
 ...
 for(const e of entries){const a=document.createElement("a");a.href=entryHash(e.id);const date=document.createElement("small");date.textContent=e.date;const title=document.createElement("strong");title.textContent=e.title;const summary=document.createElement("span");summary.textContent=e.summary;a.append(date,title,summary);a.onclick=ev=>{ev.preventDefault();$("#contents").close();go(data.entries.indexOf(e)+1);};$("#results").append(a);}
...
$("#enlarge").onclick=()=>{const e=data.entries[spread-1];$("#poster-title").textContent=e?e.date+" · "+e.title:data.book.title;$("#poster-image").src=e?asset(e.posterSrc):books[0].pages[0];$("#poster-image").alt=e?.title||data.book.title;$("#poster-image").classList.remove("actual");$("#zoom-toggle").textContent="原始尺寸";$("#poster-dialog").showModal();};
