...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-0.3.3-111111.svg" alt="Version 0.3.3">
</p>
...
- **v0.3.3**：固定参考包自动随生图附上；照片原件+压缩件归档；海报无文字插画后以随包悠哉字体合成文字；图谱和日记本统一悠哉字体。
- **v0.3.2**：每天一张海报，最多五个场景；图谱新增人物档案；日记本按日期补留白页。
...
日记本保持每张海报的原始比例完整显示：新海报采用 3:4，历史 9:16 海报使用 `contain` 置入 3:4 书页，不裁切、不拉伸。翻页舞台使用白纸、浅青横线、黑墨线和轻量干净的中性灰投影；所有交互与文字均随包使用悠哉字体。日记页不重复叠加日期和标题，只在页脚显示摘要、标签和页码；同时提供桌面双页、手机单页、目录、日期/标题/标签/角色搜索、`#entry` 深链和按日记 ID 恢复的本地阅读进度。书内人物卡与只读关系概览会跳转到独立人物图谱，并通过 `?character=<角色ID>` 自动选中节点；不把图谱 Canvas 嵌入翻页页。
...
| 日记编排 | 角色一致性 | 人物关系图 |
| --- | --- | --- |
| 居中日期/周几、居中标题和 1–5 个纵向场景（每天一张） | 复用头像、五官、比例、服装与宠物锚点 | HTML 节点拖拽、右侧人物档案与标准卡、关系编辑与智能编排 |
...
也可以附上照片、聊天记录或事件清单。Skill 会先归档照片，再生成新角色的人物形象卡供一次确认；固定画风和模板参考会随每次生图附上。海报先生成无文字插画，再用随包悠哉字体合成日期、标题和右侧备注。
...
- 黑白墨线、白纸和浅青横线；日期与标题居中，1–5 个场景从上到下排列，短备注放在右侧；每天始终只保留一张海报。
